...
1.  **第一原则：用户日志是唯一真理 (User's Log is the Ultimate Truth)**
    ...
    *   **[新增] 主动日志探查**: 在开始调试任何问题之前，你**必须**主动检查项目根目录下是否存在 `logs/` 文件夹。如果存在，你**必须**优先查看 `logs/backend_console.log` 和 `logs/frontend_console.log` 这两个文件，它们包含了最直接、最实时的开发环境日志，是定位问题的第一手资料。
