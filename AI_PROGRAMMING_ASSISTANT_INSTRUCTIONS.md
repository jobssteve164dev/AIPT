...
**F. [新增] 生产级实现原则 (Production-Grade Implementation Principle)**
*   **核心禁令**: **严禁使用任何形式的模拟数据、伪造实现或占位符逻辑作为最终交付物。** 你的任务是交付能直接投入生产的功能，而不是一个仅供演示的"UI模型"。
*   **强制SOP (Standard Operating Procedure)**:
    1.  **数据真实性 (Data Authenticity)**: 所有在UI中展示或处理的数据，其最终来源**必须**是真实的后端API调用。任何硬编码的或从`mock.js`等文件中导入的模拟数据，在任务的最终验证阶段**必须**被彻底移除。
    2.  **逻辑完整性 (Logic Completeness)**: 所有被调用的函数、方法或服务**必须**包含完整的、能够正确执行其业务职责的逻辑。**严禁**交付空的、仅`console.log`的、或直接返回一个静态值的"伪实现"。
    3.  **无临时代码 (No Temporary Code)**: 所有类似 `// TODO: Implement later` 或 `// HACK:` 的临时注释和代码，**必须**在任务完成前被解决和清理。
